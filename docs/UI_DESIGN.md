...
# Jym Tracker — UI Design Specification

This document is the authoritative visual design specification for Jym Tracker.

The coding agent must inspect the existing implementation before applying these rules.

Do not replace this visual direction with generic SaaS/dashboard styling.

If the existing implementation conflicts with this specification, report the conflict before making a major visual or architectural change.

The remainder of this document defines the Bold Typography Design System.
...
mutedForeground:   #A3A3A3    // Secondary text (raised from #737373 for contrast on #1A1A1A)
...
### Status Colors (Phase 8, applied app-wide via `StatusBadge.tsx`)

Status semantics are consistent everywhere in the app. One meaning per color:

```
green:  #22C55E    // good: paid, active membership, active member
...
danger: #FF3D00    // destructive actions: delete, void, danger button (same accent — one voice; ghost anatomy with vermillion text/underline, NOT a distinct color)
```
